...
//Created By: Kiran Saraswathy Bhavanam Radhakrishnan
...
const Graphs = ({ tasks }) => {
  useEffect(() => {
    ...
    barSvg
      ...
      .append('rect')
      .attr('x', (d, i, nodes) =>
        i === 0 ? 0 : xScale(d3.select(nodes[i - 1]).datum().value)
      )
      .attr('y', 0)
    ...
    barSvg
      ...
      .attr(
        'transform',
        `translate(0, ${barHeight - margin.top - margin.bottom})`
      )
    ...
    const legend = barSvg
      ...
      .attr(
        'transform',
        `translate(${(barWidth - margin.left - margin.right) / 2 - 60}, ${
          barHeight - margin.top - margin.bottom + 20
        })`
      );
      ...
      {tasks.length === 0 ? (
        <Box>
          ...
          <Typography variant="body1" sx={{ textAlign: 'center' }}>
            Once tasks are added, graphs will appear here to show the priority
            distribution and task completion status.
          </Typography>
        </Box>
      ) : (
        <Box
